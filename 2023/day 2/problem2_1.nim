...
# As you walk, the Elf shows you a small bag and some cubes which are either red, green, or blue. Each time you play this game, he will hide a secret number of cubes 
# of each color in the bag, and your goal is to figure out information about the number of cubes.

# To get information, once a bag has been loaded with cubes, the Elf will reach into the bag, grab a handful of random cubes, show them to you, and then put them back in 
...
# In game 1, three sets of cubes are revealed from the bag (and then put back again). The first set is 3 blue cubes and 4 red cubes; the second set is 1 red cube, 
...
proc getId(line: string): int =
    var input = line.split(":");
    return parseInt(input[0].split(" ")[1]);

proc gamePossible(line: string, max_red: int = 12, max_green: int = 13, max_blue: int = 14): bool =
    var input = line.split(":")[1];
    var grabs = input.split(";");

    for grab in grabs:
        var cubes = grab.split(",");

        for cube in cubes:
            var stripped = cube.strip();
            var num = parseInt(stripped.split(" ")[0]);
            var color = stripped.split(" ")[1];
...
let lines = readFile("input2_1.txt").splitLines();
