...
const card_to_strength = {'A': 13, 'K': 12, 'Q': 11, 'J': 10, 'T': 9, '9': 8, '8': 7, '7': 6, '6': 5, '5': 4, '4': 3, '3': 2, '2': 1}.toTable;
...
proc rank(hand: string): int =
    ...
    for c in hand:
        ...
        # Four of a kind
            ...
            break;
        # Full House
        elif(count == 3 and rank == 2 or count == 2 and rank == 4):
            rank = 5;
            break;
        # Three of a kind
        elif(count == 3 and not seen.contains(c)):
            rank = 4;
        # Two pair
        elif(count == 2 and rank == 2  and not seen.contains(c)):
            rank = 3;
        # One pair
        elif(count == 2  and not seen.contains(c)):
            rank = 2;
...
let lines = readFile("input7_1.txt").splitLines();
var total = 0;
